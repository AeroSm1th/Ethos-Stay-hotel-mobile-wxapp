...
### 修复
- **首页设施标签交互优化** (2026-02-03)
  - 修改了首页设施标签的交互方式
  - **修改前**：点击标签后立即筛选推荐酒店（每次点击都会发送网络请求）
  - **修改后**：点击标签只切换高亮状态，不立即筛选
  - 用户可以先选择多个标签，然后点击"搜索酒店"按钮一次性筛选
  - 优势：
    - 减少不必要的网络请求
    - 用户体验更好（可以一次选择多个标签）
    - 避免频繁的加载状态切换
...
- **列表页设施标签初始化** (2026-02-03)
  - 修复了从首页跳转到列表页时，设施标签没有高亮显示的问题
  - 现在会从 URL 参数中正确解析 `tags` 参数
  - 解析后的标签会自动设置为选中状态（高亮显示）
  - 列表页加载时会根据选中的标签进行本地筛选
  - 用户体验：
    - 首页选择设施标签 → 点击"搜索酒店" → 列表页显示选中的标签（高亮）
    - 列表页可以继续点击标签进行筛选（即时响应，无需网络请求）
